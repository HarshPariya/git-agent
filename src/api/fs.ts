...
export async function browseFilesystemHandler(request: Request, response: Response, next: NextFunction): Promise<void> {
  try {
    ...
    if (!rawPath) { const workspaceParent = path.dirname(process.cwd()); targetPath = fs.existsSync(workspaceParent) ? workspaceParent : process.cwd(); }
    else if (rawPath === "~" || rawPath.startsWith("~/") || rawPath.startsWith("~\\")) targetPath = path.resolve(homedir, rawPath.replace(/^~[/\\]?/, ""));
    else if (rawPath.length === 2 && /^[a-zA-Z]:$/.test(rawPath)) targetPath = `${rawPath}\\`;
    else targetPath = path.resolve(rawPath);
    try { const stat = await fs.promises.stat(targetPath); if (!stat.isDirectory()) targetPath = path.dirname(targetPath); } catch { targetPath = process.cwd(); }
    const currentIsGit = fs.existsSync(path.join(targetPath, ".git"));
    const parsed = path.parse(targetPath); const parentPath = targetPath === parsed.root ? null : path.dirname(targetPath);
    const directories: DirectoryItem[] = []; const files: FileItem[] = [];
    try {
      const entries = await fs.promises.readdir(targetPath, { withFileTypes: true });
      for (const entry of entries) {
        if (entry.isDirectory() && (entry.name.startsWith(".") || SKIP_DIRS.has(entry.name))) continue;
        if (SKIP_FILES.has(entry.name)) continue;
        if (entry.isDirectory()) { let isGit = false; try { isGit = fs.existsSync(path.join(path.join(targetPath, entry.name), ".git")); } catch { isGit = false; } directories.push({ name: entry.name, path: path.join(targetPath, entry.name), isGitRepo: isGit }); }
        else if (entry.isFile()) { const ext = path.extname(entry.name).toLowerCase(); if (BINARY_EXTS.includes(ext)) continue; let sizeBytes = 0; try { sizeBytes = (await fs.promises.stat(path.join(targetPath, entry.name))).size; } catch { sizeBytes = 0; } files.push({ name: entry.name, path: path.join(targetPath, entry.name), ext: ext || "(no ext)", sizeBytes }); }
      }
    ...
    directories.sort((a, b) => { if (a.isGitRepo && !b.isGitRepo) return -1; if (!a.isGitRepo && b.isGitRepo) return 1; return a.name.localeCompare(b.name, undefined, { sensitivity: "base" }); });
    files.sort((a, b) => a.name.localeCompare(b.name, undefined, { sensitivity: "base" }));
    const shortcuts: { name: string; path: string }[] = [];
    const cwd = process.cwd(); const workspaceParent = path.dirname(cwd);
...
export async function resolveFolderHandler(request: Request, response: Response, next: NextFunction): Promise<void> {
  try {
    ...
    const homedir = os.homedir();
    const variants = Array.from(new Set([folderName, folderName.replace(/\s+/g, "-"), folderName.replace(/-/g, " "), folderName.replace(/[_\s-]+/g, "")])).filter(Boolean);
    const searchRoots: string[] = [];
    if (currentBrowsedPath && fs.existsSync(currentBrowsedPath)) { searchRoots.push(currentBrowsedPath); const parent = path.dirname(currentBrowsedPath); if (parent && fs.existsSync(parent) && !searchRoots.includes(parent)) searchRoots.push(parent); }
    for (const p of [homedir, path.join(homedir, "Desktop"), path.join(homedir, "Documents"), path.join(homedir, "Downloads"), path.join(homedir, "Projects"), path.join(homedir, "Developer"), path.join(homedir, "Workspace"), path.join(homedir, "source", "repos"), path.join(homedir, "repos")]) { if (!searchRoots.includes(p)) searchRoots.push(p); }
    if (process.platform === "win32") { for (const drive of getAvailableDrives()) { if (!searchRoots.includes(drive)) searchRoots.push(drive); } } else { if (!searchRoots.includes("/")) searchRoots.push("/"); }
    ...
    response.status(200).json({ resolvedPath: best, folderName: best ? path.basename(best) : folderName, exists: Boolean(best && fs.existsSync(best)), isGitRepo: Boolean(best && fs.existsSync(path.join(best, ".git"))), candidates: scoredCandidates.map((c) => c.path) } as ResolveFolderResult);
  } catch (error) { next(error); }
}
...
export async function pickNativeDialogHandler(_request: Request, response: Response, next: NextFunction): Promise<void> {
  try {
    ...
    if (!selectedPath) { response.status(200).json({ cancelled: true } as PickNativeDialogResult); return; }
    const exists = fs.existsSync(selectedPath);
    response.status(200).json({ path: selectedPath, folderName: path.basename(selectedPath) || selectedPath, isGitRepo: exists && fs.existsSync(path.join(selectedPath, ".git")), cancelled: false } as PickNativeDialogResult);
  } catch (error) { next(error); }
}
...
export async function openInOsHandler(request: Request, response: Response, next: NextFunction): Promise<void> {
  try {
    ...
    if (repoId) { const repoPath = getExecutionPath(repoId); fullPath = filePath ? (path.isAbsolute(filePath) ? filePath : path.resolve(repoPath, filePath)) : repoPath; }
    else if (filePath && !path.isAbsolute(filePath)) fullPath = path.resolve(process.cwd(), filePath);
    ...
    if (process.platform === "win32") { if (mode === "reveal") { execFile("explorer.exe", [isDir ? fullPath : `/select,${fullPath}`], () => { }); } else { execFile("code", [fullPath], (err) => { if (err) execFile("cmd.exe", ["/c", "start", "", fullPath], () => { }); }); } }
    else if (process.platform === "darwin") { execFile("open", [mode === "reveal" ? "-R" : "", fullPath].filter(Boolean), () => { }); }
    else { execFile("xdg-open", [isDir ? fullPath : path.dirname(fullPath)], () => { }); }
