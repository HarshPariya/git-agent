...
export const addProtectedBranchHandler = async (request: Request, response: Response, next: NextFunction): Promise<void> => {
  try {
    const context = getTenantContext(request);
    const repositoryId = request.params.id as string;
    const branchName = requireString(request.body, "branchName");
    const branch = repositoryStore.addProtectedBranch(repositoryId, context.tenantId, branchName);
