...
const normalize = (s: string): string =>
  s.toLowerCase().replace(/[^a-z0-9_$]+/g, " ").trim();
...
const calculateExactNameBonus = (query: string, result: HybridSearchResult): number => {
  ...
  const expandedQuery = new Set(
    nq.split(/\s+/).filter(Boolean).flatMap((q) => [q, ...(QUERY_EXPANSIONS[q] ?? [])])
  );
  ...
  return matchedParts === nameParts.length
    ? 0.85
    : matchedParts > 0
      ? 0.4 * (matchedParts / nameParts.length)
      : 0;
};
...
const calculateTypeBonus = (r: HybridSearchResult): number =>
  TYPE_BONUS_MAP[r.chunk?.type ?? ""] ?? 0;
