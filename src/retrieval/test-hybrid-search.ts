...
async function main() {
  ...
  if (connected) await initializeSchema();
  else console.warn("PostgreSQL not available. Running hybrid search in standalone AST & graph mode.");
  ...
  if (connected) { console.log("Persisting vector index to PostgreSQL..."); await upsertChunks("ai-chatbot", chunks); }

  const query = process.argv.slice(2).join(" ") || "Where is normalizeId used?";
  console.log(`\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\nQuery: "${query}"\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n`);

  if (connected) vectorResults = await pgVectorSearch(query, { repository: "ai-chatbot", limit: 15 });
  ...
  results.forEach((result, index) => {
    ...
    if (result.graphDepth !== undefined) console.log(`   Graph depth: ${result.graphDepth}`);
    console.log();
  });
}

main().catch((error) => { console.error("Hybrid search failed:"); console.error(error); process.exitCode = 1; }).finally(async () => { await closeDatabase(); });
