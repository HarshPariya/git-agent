/**
 * Context Builder
 * Aggregates multi-source debugging context:
 * Git + Code (GraphRAG) + Issues + PRs + Stack Traces
 */
...
export class ContextBuilder {
  ...
  private async buildGitContext(repositoryId: string): Promise<GitContext> {
    try {
      ...
      const recentCommits = l
        .map((c) => `${c.shortHash} ${c.author} — ${c.message}`)
        .join("\n");
      const diffText = d
  ...
  /**
   * Serializes context to a concise LLM prompt string
   * Bounded to avoid token blowout
   */
  toPrompt(ctx: DebugContext): string {
    ...
    if (ctx.stackTrace) {
      lines.push("", "=== STACK TRACE ===", ctx.stackTrace.slice(0, 2000));
    }

    if (ctx.issueText) {
      lines.push("", "=== ISSUE ===", ctx.issueText.slice(0, 1000));
    }

    if (ctx.prText) {
      lines.push("", "=== PR ===", ctx.prText.slice(0, 1000));
    }

    if (ctx.git.recentCommits) {
      lines.push("", "=== RECENT COMMITS ===", ctx.git.recentCommits.slice(0, 1500));
    }

    if (ctx.git.diff) {
      lines.push("", "=== CHANGED FILES ===", ctx.git.diff.slice(0, 1500));
    }

    if (ctx.code.searchResults) {
      lines.push("", "=== CODE GRAPH ===", ctx.code.searchResults.slice(0, 1000));
    }

    if (ctx.code.symbols.length > 0) {
      lines.push("", "=== KEY SYMBOLS ===", ctx.code.symbols.join(", "));
    }
