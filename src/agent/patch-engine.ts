/**
 * Patch Engine
 * Minimal, reversible patch applier with pre-apply file-state validation.
 * Supports file snapshots, rollback, unified diff generation, and safe file writes.
 */
...
export interface PatchBackup {
  ...
  readonly files: ReadonlyMap<string, string>; // relativePath -> originalContent
...
// In-memory backup store for instant rollback
...
/**
 * Validates that all target files exist (or can be created) and are within the repo boundary
 */
export async function validatePrePatch(
  ...
  for (const change of changes) {
    ...
    if (!fullPath.startsWith(resolvedRepo)) {
      errors.push(`File path traverses outside repository: ${change.filePath}`);
      continue;
    }
  }
...
/**
 * Creates a snapshot backup of files before any patch is applied
 */
export async function createPatchBackup(
  repoPath: string,
  filePaths: readonly string[],
  description: string = "Pre-patch backup",
): Promise<PatchBackup> {
...
/**
 * Applies minimal reversible patch to repository files with safety backup
 */
export async function applyPatch(
  repoPath: string,
  changes: readonly PatchFileChange[],
  description: string = "Agent Fix Patch",
): Promise<PatchResult> {
  ...
  // Create backup before touching any file
  ...
  try {
    for (const change of changes) {
      ...
      diffParts.push(`--- a/${change.filePath}\n+++ b/${change.filePath}`);
      const origLines = change.originalContent.split("\n");
      const newLines = change.newContent.split("\n");
      diffParts.push(`@@ -1,${origLines.length} +1,${newLines.length} @@`);
      origLines.forEach((l) => diffParts.push(`-${l}`));
      newLines.forEach((l) => diffParts.push(`+${l}`));
    }

    // Also get git diff if possible
    let gitDiff = "";
    try {
    ...
    return {
      ...
      appliedFiles: [],
      diff: "",
      error: `Failed to apply patch: ${err.message}. Automatically rolled back.`,
    };
  }
}

/**
 * Reverts a patch given its backup ID
 */
export async function revertPatch(backupId: string): Promise<RevertResult> {
  ...
  if (!backup) {
    return {
      success: false,
      restoredFiles: [],
      error: `Backup ${backupId} not found in store`,
    };
  }
  ...
  try {
    for (const [relPath, originalContent] of backup.files.entries()) {
      ...
      if (originalContent === "") {
        // Was a new file created by patch; remove it
        try {
...
/**
 * Lists all active patch backups
 */
...
/**
 * Applies a unified diff or targeted line replacements to original file content
 */
export function applyDiffHunk(original: string, patch: string): string {
  ...
  // If not diff markers, but not an existing file, or is a complete file replace
