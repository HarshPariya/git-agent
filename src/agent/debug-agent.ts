...
export class DebugAgentPipeline {
  async startSession(
    ...
    logger.info("Debug session started", {
      ...
      metadata: { sessionId, repositoryId, mode },
    });
    ...
    if (listeners) {
      for (const listener of listeners) {
        try {
          ...
          logger.error("SSE listener dispatch error", {
            operation: "sse-dispatch",
  ...
  async executeStep(
    ...
    const updateTotal =
      session.totalSteps > stepIndex + 1 ? session.totalSteps : stepIndex + 1;
    const updatedSession: DebugSession = {
      ...session,
      currentStep: stepIndex + 1,
      totalSteps: updateTotal,
      steps: [...session.steps, step],
    };
    debugSessions.set(context.sessionId, updatedSession);

    this.emitEvent(context.sessionId, {
      type: "step",
    ...
    try {
      ...
      const completedSession: DebugSession = {
        ...updatedSession,
        totalSteps: updateTotal,
        steps: updatedSession.steps.map((s, i) =>
          i === stepIndex ? completedStep : s,
        ),
      };
  ...
  async generatePatch(
    ...
    if (!available) {
      logger.warn("LLM not available, returning fallback patches", {
        operation: "debug-patch-gen",
        metadata: { sessionId: context.sessionId },
      });
      return this._generateFallbackPatches(
        rootCause,
        affectedFiles,
        codeContext,
      );
    ...
    try {
      ...
      return candidates.length > 0
        ? candidates
        : this._generateFallbackPatches(rootCause, affectedFiles, codeContext);
    } catch (err) {
  ...
  async runCritic(
    ...
    if (!available) {
      return {
        approved: true,
        feedback:
          "LLM critic not available — patch auto-approved with low confidence",
        score: 0.5,
      };
    }
