...
export interface PercentileMetrics { p50Ms: number; p90Ms: number; p95Ms: number; p99Ms: number; }
export interface SlowQueryRecord { timestamp: string; query: string; repository: string; totalMs: number; vectorMs: number; graphMs: number; rerankMs: number; }
export interface RepositoryMetricSummary { repository: string; totalRequests: number; failedRequests: number; avgLatencyMs: number; }

export interface SystemMetrics {
  healthScore: { status: "EXCELLENT" | "GOOD" | "DEGRADED" | "CRITICAL"; scorePercentage: number; description: string };
  retrieval: { totalRequests: number; failedRequests: number; totalLatencyMs: number; avgLatencyMs: number; percentiles: PercentileMetrics; vectorLatencyMs: number; graphLatencyMs: number; rerankLatencyMs: number };
  database: { totalQueries: number; failedQueries: number; poolTotalConnections: number; poolIdleConnections: number; poolWaitingCount: number };
  embeddings: { modelLoadTimeMs: number; totalEmbeddings: number; failedEmbeddings: number; avgLatencyMs: number };
  graphCache: { hits: number; misses: number; hitRate: string };
  indexing: { totalIndexedFiles: number; totalDeletedFiles: number; lastIndexingDurationMs: number };
...
class MetricsCollector {
  ...
  public recordRetrieval(totalMs: number, vectorMs: number, graphMs: number, rerankMs: number, success = true, queryText = "", repository = "ai-chatbot"): void {
    ...
    if (totalMs >= this.SLOW_QUERY_THRESHOLD_MS && queryText) {
      this.slowQueries.push({ timestamp: new Date().toISOString(), query: queryText, repository, totalMs, vectorMs, graphMs, rerankMs });
      if (this.slowQueries.length > 50) this.slowQueries.shift();
    }
  }

  public recordDbQuery(success = true): void { this.dbQueries++; if (!success) this.failedDbQueries++; }
  public recordCacheHit(hit: boolean): void { hit ? this.cacheHits++ : this.cacheMisses++; }

  public recordRagMetrics(metrics: {
    retrievalMode: string; retrievedCandidates: number; rerankedCandidates: number; sentToLLM: number;
    ragContextTokens: number; memoryTokens: number; systemTokens: number; queryTokens: number;
  ...
  private calculatePercentiles(): PercentileMetrics {
    ...
    const getPercentile = (p: number) => sorted[Math.max(0, Math.ceil((p / 100) * sorted.length) - 1)] ?? 0;
    return { p50Ms: getPercentile(50), p90Ms: getPercentile(90), p95Ms: getPercentile(95), p99Ms: getPercentile(99) };
  }

  public calculateHealthScore(): SystemMetrics["healthScore"] {
    if (this.retrievalRequests === 0) return { status: "EXCELLENT", scorePercentage: 100, description: "System idle and fully healthy." };
    const percentiles = this.calculatePercentiles();
    let score = 100 - (this.failedRetrievals / this.retrievalRequests) * 200;
    ...
    return { status, scorePercentage: score, description: `RAG retrieval SLA health score at ${score}% (${status}). P95 latency: ${percentiles.p95Ms}ms.` };
  }

  public getMetrics(embeddingStats?: { modelLoadTimeMs: number; totalEmbeddings: number; failedEmbeddings: number; averageLatencyMs: number }): SystemMetrics {
    ...
    for (const [repo, data] of this.repoMetricsMap.entries()) {
      perRepoObj[repo] = { repository: repo, totalRequests: data.total, failedRequests: data.failed, avgLatencyMs: data.total > 0 ? Math.round(data.totalLatencyMs / data.total) : 0 };
    }
    const avgLatency = (total: number, count: number) => count > 0 ? Math.round(total / count) : 0;
    return {
      healthScore: this.calculateHealthScore(),
      retrieval: {
        totalRequests: this.retrievalRequests, failedRequests: this.failedRetrievals,
      ...
      database: {
        totalQueries: this.dbQueries, failedQueries: this.failedDbQueries,
        poolTotalConnections: pool.totalCount ?? 0, poolIdleConnections: pool.idleCount ?? 0, poolWaitingCount: pool.waitingCount ?? 0,
      },
      embeddings: {
        modelLoadTimeMs: embeddingStats?.modelLoadTimeMs ?? 0, totalEmbeddings: embeddingStats?.totalEmbeddings ?? 0,
        failedEmbeddings: embeddingStats?.failedEmbeddings ?? 0, avgLatencyMs: embeddingStats?.averageLatencyMs ?? 0,
      },
      graphCache: { hits: this.cacheHits, misses: this.cacheMisses, hitRate: hitRatePercent },
      indexing: { totalIndexedFiles: this.indexedFiles, totalDeletedFiles: this.deletedFiles, lastIndexingDurationMs: this.lastIndexingDurationMs },
      slowQueries: this.slowQueries,
      perRepository: perRepoObj,
    };
  }

  public getPrometheusFormat(embeddingStats?: { modelLoadTimeMs: number; totalEmbeddings: number; failedEmbeddings: number; averageLatencyMs: number }): string {
