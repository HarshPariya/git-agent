...
export class ConflictAnalyzer {
  async analyzeRepository(repositoryPath: string): Promise<ConflictAnalysis> {
    ...
    const incomingBranch = await runGitTrimmed("cat .git/MERGE_HEAD 2>/dev/null | head -c 8 || echo 'incoming'", repositoryPath);
  ...
  private parseConflictMarkers(filePath: string, content: string): ConflictMarker[] {
    ...
    for (let idx = 0; idx < lines.length; idx++) {
      ...
      if (line.startsWith(CONFLICT_LINE_PREFIXES.base)) { state = "base"; continue; }
      if (line.startsWith(CONFLICT_LINE_PREFIXES.separator)) { state = "theirs"; continue; }

      if (line.startsWith(CONFLICT_LINE_PREFIXES.end)) {
        markers.push({ filePath, ourLines: [...ourLines], theirLines: [...theirLines], baseLines: [...baseLines], startLine, endLine: idx + 1 });
        state = "idle";
        continue;
      }

      switch (state) {
        case "ours": ourLines.push(line); break;
        case "base": baseLines.push(line); break;
        case "theirs": theirLines.push(line); break;
      }
    }
  ...
  private async resolveWithLlm(cf: ConflictFile): Promise<ConflictResolution> {
    try {
      const conflictBlocks = cf.markers.map((m, i) => (
      ...
      const response = await callLlm([
        ...
        {
          role: "user",
          content: `Resolve merge conflict in ${cf.filePath} (${cf.markers.length} conflict(s)):\n${conflictBlocks}\n\n` +
      ...
      const parsed = JSON.parse(jsonMatch[0]) as { resolution?: string; strategy?: ConflictResolution["strategy"]; explanation?: string; confidence?: number; warnings?: string[] };
      return {
  ...
  private resolveDeterministic(cf: ConflictFile): ConflictResolution {
    const resolvedLines = cf.markers.flatMap((m) => [...m.ourLines, ...m.theirLines.filter((l) => !m.ourLines.includes(l))]);
    return {
      filePath: cf.filePath,
      ...
      confidence: 0.5,
      warnings: ["LLM not available — manual review of this resolution is strongly recommended.", "Deterministic merge may not preserve semantic intent."],
    };
  }
  ...
  async resolveAllConflicts(repositoryPath: string): Promise<{ success: boolean; analysis: ConflictAnalysis; appliedCount: number; errors: string[] }> {
    ...
    for (const resolution of analysis.resolutions) {
      ...
      if (result.success) { appliedCount++; continue; }
