...
const safeExec = async (cmd: string, cwd: string): Promise<{ stdout: string; stderr: string }> =>
  execAsync(cmd, { cwd, timeout: 30_000 });
...
export async function safeCommit(repositoryId: string, tenantId: string, commitMessage: string, filesToStage?: string[]): Promise<CommitResult> {
  let repoPath: string;
  try { const repo = repositoryStore.getRepository(repositoryId, tenantId); if (!repo) return { success: false, message: "Repository not found", error: "Repository not found in store" }; repoPath = repo.localPath || (repo.url ? repo.url.replace("file://", "") : ""); }
  catch (err) { return { success: false, message: "Repository not found", error: err instanceof Error ? err.message : "Unknown error" }; }

  try {
    if (filesToStage && filesToStage.length > 0) { for (const f of filesToStage) await safeExec(`git add "${f}"`, repoPath); }
    else await safeExec("git add -A", repoPath);
    const { stdout: statusOut } = await safeExec("git status --porcelain", repoPath);
    if (!statusOut.trim()) return { success: false, message: "Nothing to commit — working tree clean." };
  ...
  } catch (err) { return { success: false, message: "Commit failed", error: err instanceof Error ? err.message : String(err) }; }
}
...
export async function safePush(repositoryId: string, tenantId: string, branch?: string, options: { dryRun?: boolean; force?: boolean } = {}): Promise<CommitResult> {
  ...
  try {
    ...
    return { success: true, message: options.dryRun ? `Dry run push succeeded. Branch: ${branch ?? "current"}` : `Pushed successfully to origin/${branch ?? "current branch"}`, commitHash: stdout.slice(0, 50) };
  } catch (err) {
