...
export function validateBranchName(name: string): string {
  ...
  if (!/^[a-zA-Z0-9._/-]+$/.test(trimmed))
    throw new Error(
      `Invalid branch name: "${trimmed}". Only alphanumeric, dots, underscores, slashes, and hyphens allowed.`,
    );
  if (trimmed.includes("..")) throw new Error(`Invalid branch name: "${trimmed}". Cannot contain consecutive dots.`);
  if (trimmed.startsWith(".") || trimmed.endsWith("."))
    throw new Error(`Invalid branch name: "${trimmed}". Cannot start or end with a dot.`);
...
export function validateRemoteName(name: string): string {
  ...
  if (!/^[a-zA-Z0-9._-]+$/.test(trimmed))
    throw new Error(`Invalid remote name: "${trimmed}". Only alphanumeric, dots, underscores, and hyphens allowed.`);
  if (trimmed.startsWith(".") || trimmed.startsWith("-"))
    throw new Error(`Invalid remote name: "${trimmed}". Cannot start with a dot or hyphen.`);
...
export function validateFilePath(filePath: string): string {
  ...
  if (/[|;&$`(){}[\]!#~<>\\]/.test(trimmed))
    throw new Error(`Invalid file path: "${trimmed}". Contains shell metacharacters.`);
