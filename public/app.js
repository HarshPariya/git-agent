...
function populateRepoDropdowns() {
  ...
  const options = state.repositories.map((r) => {
...
function openCreatePRModal(repoId) {
  const targetRepoId = repoId || state.currentSession?.repositoryId || state.activeRepository?.id || (state.repositories[0]?.id);
  const repo = state.repositories.find((r) => r.id === targetRepoId) || state.activeRepository;
  ...
  const currentBranch = repo?.currentBranch || state.activeRepository?.currentBranch || "feature/git-agent";
  const defaultBranch = repo?.defaultBranch || state.activeRepository?.defaultBranch || "development";
  ...
  if (descInput) {
    const summary = state.currentFixPlan?.rootCause || "Production Git Debugging Agent enhancements: clean working tree states, executive post-push summary cards, Groq Conventional Commit generator, and Windows Git Engine fixes.";
    const impact = state.currentFixPlan?.estimatedImpact || "All 5 automated test suites passed cleanly (121 assertions, 0 failed).";
...
async function submitCreatePR() {
  const repoId = state.currentSession?.repositoryId || state.activeRepository?.id || (state.repositories[0]?.id) || "repo-ai-chatbot";
  const repo = state.repositories.find((r) => r.id === repoId) || state.activeRepository;
  ...
  try {
    ...
    // Create in Agent backend
    const res = await api.createPR(repoId, title, sourceBranch, targetBranch, description);

    // If GitHub token is connected and cloud repo, also attempt GitHub PR
    if (state.gitHubConnected && repo?.url && repo.url.includes("github.com")) {
      ...
      container.innerHTML = `
        <div class="empty-state" style="padding:48px 24px;text-align:center">
          ...
          <div style="display:flex;gap:12px;justify-content:center;flex-wrap:wrap">
            ...
            <a href="${escapeHtml(ghUrl)}" target="_blank" rel="noreferrer" class="btn btn-secondary" style="display:inline-flex;align-items:center;gap:6px">
        ...
        (pr) => {
          ...
          <div style="display:flex;align-items:center;justify-content:space-between;padding:16px 20px;border-bottom:1px solid var(--c-border-subtle);gap:16px">
            ...
            <div style="display:flex;gap:8px;align-items:center;flex-shrink:0">
              ...
              ` : ""}
              ${ghUrl ? `
                <a href="${escapeHtml(ghUrl)}" target="_blank" rel="noreferrer" class="btn btn-secondary btn-sm" title="View on GitHub">
                  GitHub ↗
                </a>
              ` : ""}
            </div>
