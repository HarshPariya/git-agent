...
async function runApiTests() {
  ...
  try {
    ...
    const classifyRes = await fetch(`${baseUrl}/api/git/classify/push`, { headers: { Authorization: `Bearer ${token}` } });
    ...
    const fsRes = await fetch(`${baseUrl}/api/fs/browse?path=${cwdEncoded}`, { headers: { Authorization: `Bearer ${token}` } });
